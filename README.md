...
One more thing worth knowing: this VAE overflows to NaN (solid black output) when decoded in plain fp16. It flags that itself via `force_upcast: true` in its config. Both `generation/model.py` and `inpainting/model.py`'s `decode_latents` already check for that flag and decode in fp32 when needed, so there's nothing extra to configure — just know it's why that logic is there.
